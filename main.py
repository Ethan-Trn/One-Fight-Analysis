...
def AthleteScraper(i):
    ...
    with open(stats_file, mode='a', newline='', encoding='utf-8') as s_file, \
        ...
        if not stats_exists:
            stats_writer.writerow(['Name', 'Weight (kg)', 'Height (cm)', 'Country', 'Age', 'Team', 'Sport', 'Gender','WIN', 'LOSS','W TKO','W Submission','W Disqualification','W Unanimous Decision', 'W Split Decision','L TKO','L Submission','L Disqualification','L Unanimous Decision','L Split Decision'])

        if not fights_exists:
            fights_writer.writerow(['Fighter', 'Opponent', 'Result'])

        for link in AllLinks:
            href = link.get("href")
            try:
                matchups = Matchup(href)
                fights_writer.writerows(matchups)
                ...
                stats_writer.writerow(stat_data + [WIN, LOSS, w_tko, w_submission, w_disqualification, w_unanimous_decision, w_split_decision, l_tko, l_submission, l_disqualification, l_unanimous_decision, l_split_decision])
        ...
        if index != -1:
            ...
            if athletes:
                for athlete in athletes:
                    weight = OpponentWeight(athlete.get("href"))
                    if weight != -1:
                        break
            else:
                weight = -1
